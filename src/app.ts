...
hidWalletInstance
  .generateWallet({ mnemonic: HID_WALLET_MNEMONIC })
  .then(async () => {
    hidWalletInstance.offlineSigner.getAccounts().then(console.log);

    const hypersign: IHypersignAuth = new HypersignAuth(
      server,
    ...
    const mnemonic_EnglishMnemonic: EnglishMnemonic =
    ...
    const kmsVault = await kmsVaultManager.createVault(
      kmsVaultWallet,
      EDV_ID,
      EDV_BASE_URL
    );
    ...
    app.post("/share", async (req, res) => {
      try {
        ...
        const issuerVerificationMethodId =
          vp.verifiableCredential[0].proof.verificationMethod;
